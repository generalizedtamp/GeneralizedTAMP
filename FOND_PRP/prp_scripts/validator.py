...
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser import grounder
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.formula import *
from GeneralizedTAMP.FOND_PRP.prp_scripts.normalizer import flatten

import networkx as nx
from networkx.drawing.nx_pydot import write_dot
import GeneralizedTAMP.FOND_PRP.prp_scripts.validators.prp as val
import sys
import pickle 
...
class State:
    ...
    def __str__(self):
        return str([str(f) for f in self.fluents])

    def is_goal(self, goal):
        val = set(list(filter(lambda x: x>=0, list(goal)))) <= self.fluents
        for q in list(filter(lambda x: x<0, list(goal))):
            if -q in self.fluents:
                return False
        return val
...
def validate(dfile, pfile, sol, save_dir):
    ...
    write_dot(G, save_dir+"/graph.dot")
    with open(save_dir+'/policy_graph.pkl', 'wb') as handle:
...
def _convert_conjunction(mapping, conj):
    if isinstance(conj, And):
        return [mapping[str(f).lower()] for f in list(filter(lambda x: '=' not in str(x), conj.args))]
...
if __name__ == '__main__':
    try:
        (dom, prob, sol, interp, save_dir) = os.sys.argv[1:]
    except:
        print("\nError with input.")
        print(USAGE_STRING)
        os.sys.exit(1)

    validate(dom, prob, sol, save_dir)

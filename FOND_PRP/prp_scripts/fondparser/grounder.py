from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.parser import Problem
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.action import Action
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.formula import Primitive, Forall, When, And
...
class GroundProblem(Problem):
    ...
    def _partial_ground_formula(self, formula, assignment, fluent_dict, debug=False):
        """
        Inputs:
        ...
        if isinstance(formula, Primitive):
            ...
            returning = type(formula)([self._partial_ground_formula(arg, assignment, fluent_dict, debug=debug) for arg in formula.args])
            return returning
    ...
    def _action_to_operator(self, action, assignment, fluent_dict):
        ...
        op_effect = self._partial_ground_formula(action.effect, assignment, fluent_dict, debug=True)
    ...
    def _create_operators(self, fluent_dict):
        ...
        for a in self.actions:
            ...
            for valuation in val_generator:
                ...
                print([o.effect for o in list(self.operators)])

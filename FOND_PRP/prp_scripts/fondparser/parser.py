...
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.formula import Formula, And, Primitive, Forall, When, Xor, Not, Oneof, Or
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.action import Action
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.predicate import Predicate
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.pddl_tree import PDDL_Tree
from GeneralizedTAMP.FOND_PRP.prp_scripts.fondparser.utils import PDDL_Utils
...
class Problem(object):
    ...
    def _parse_problem(self, f_problem):
        ...
        if len(parse_tree[":goal"].children) == 1 and parse_tree[":goal"].children[0].name == "and":
            ...
            for c in parse_tree[":goal"].children[0].children:
                if(c.name == "not"):
                    components.append(Not([Primitive(self.to_fluents(c.children[0]))]))
                else:
                    components.append(Primitive(self.to_fluents(c)))

            self.goal = And(components)
        else:
            print("Goal 2")
    ...
    def to_predicate(self, node, f='predicate', map=None):
        ...
        if map is None:
            if 'predicate' == f:
                return Predicate(str(node.name), args)
            elif 'fluent' == f:
                return Predicate(str(node.name), args=None, ground_args=args)
        else:
            new_args = []
            for v, t in args:
                if v in map:
